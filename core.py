...
class VkTools:
    ...
    def get_profile_info(self, user_id):
        try:
            info, = self.vkapi.method('users.get',
                                      {'user_id': user_id,
                                       'fields': 'city, sex, relation, country, screen_name, bdate'
                                       }
                                      )
        ...
        result = {'name': (info['first_name'] + ' ' + info['last_name']) if
                  ...
                  'country': info.get('country')['title'] if info.get('country') is not None else None,
            ...
            users = self.vkapi.method('users.search',
                                      {
                                          'count': 50,
                                          'offset': offset,
                                          'hometown': params['city'] if params['city'] is not None else '',
    ...
    def get_photos(self, id):
        ...
        return result[:3]  # максимальное количество лайков и комментов
